...
async function metaAudienceSources(m: any) {
  ...
  for (const pg of out.pages.slice(0, 10)) {
    try { const r = await fetch(`${base}/${pg.id}/videos?fields=id,title,description,length,created_time,picture,permalink_url&limit=50&access_token=${token}`); const j = await r.json(); if (!j.error) (j.data || []).forEach((v: any) => out.videos.push({ id: v.id, name: v.title || (v.description ? String(v.description).slice(0, 45) : "") || (v.created_time ? v.created_time.slice(0, 10) : v.id), page: pg.id, account: pg.account, thumb: v.picture || "", url: v.permalink_url ? (String(v.permalink_url).startsWith("http") ? v.permalink_url : "https://www.facebook.com" + v.permalink_url) : ("https://www.facebook.com/" + v.id), len: v.length ? Math.round(v.length) : null, created: v.created_time || "" })); } catch (_e) { /* */ }
